...
abstract class HyperloopUtil {
    ...
    /**
     * Mapping from Boxed classes to the primitive types they wrap.
     */
     ...
     * @return
     */
    static Object[] wrapArguments(Object[] args) {
     ...
     * @return
     */
    static Object wrap(Object result) {
        if (result == null) {
            return null;
        }
        ...
        if (isKnownType(result)) {
            // FIXME Add some caching mechanism so we don't rewrap the same
            // object/class in multiple proxies?
        ...
        // Wrap in proxy
        // FIXME Should we keep track of proxies wrapping native objects so we
        // can return the exact same instance each time?
        // i.e. if we already have a proxy wrapping a specific instance of a
        // button, does it matter if we generate a new proxy around the same
        // button later for a callback?
        InstanceProxy p = new InstanceProxy(result);
        return p;
    }
    ...
    static Object unwrap(Object object) {
        ...
        // Native code handles unwrapping JS proxy to the underlying Java BaseProxy
        // see https://github.com/appcelerator/titanium_mobile/blob/master/android/runtime/v8/src/native/TypeConverter.cpp#L628
    ...
    /**
     * Used to generate a human-readable version of an argument array for error
     * messages.
     * 
     * @param arguments
     * @return
     */
    static String stringify(Object[] arguments) {
        StringBuilder builder = new StringBuilder("[");
        final int argCount = (arguments == null) ? 0 : arguments.length;
        if (argCount > 0) {
            builder.append(arguments[0] == null ? "null" : arguments[0].toString());
            for (int i = 1; i < argCount; i++) {
                builder.append(", ")
                        .append(arguments[i] == null ? "null" : arguments[i].toString());
            }
        }
        builder.append("]");
